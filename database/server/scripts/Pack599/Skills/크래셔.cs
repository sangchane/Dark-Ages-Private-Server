...
{
    /// <summary>
    /// 크래셔 — 5.99 `전사(비전직).txt` 의 SKILL_크래셔 을 그대로 옮긴 것.
    /// </summary>
    /// <remarks>
    /// 손으로 고치지 말 것. `scripts/build-pack-abilities.py` 가 다시 만든다.
    /// </remarks>
    [Script("크래셔", "5.99표")]
    public class SkillD06CB798C154 : SkillScript
    {
        public SkillD06CB798C154(Skill skill) : base(skill)
        {
        }
        ...
        public override void OnUse(Sprite sprite)
        {
            ...
            v_damage = ((V)(p.Call("get_basevita", v_myid)) * (V)((V)8L));
            ...
            if (V.T(V.B(!V.T(v_target))))
            {
                if (V.T(V.B(V.T(((V)(p.Call("istype", p.Call("get_front_char", v_myid))) == (V)((V)3L))) && V.T(p.Call("get_map_pk")))))
                {
                    ...
                    p.Call("effect", v_target, (V)299L, (V)299L, (V)100L);
