...
{
    /// <summary>
    /// 타겟어택 — 5.99 `전사(비전직).txt` 의 SKILL_타겟어택 을 그대로 옮긴 것.
    /// </summary>
    /// <remarks>
    /// 손으로 고치지 말 것. `scripts/build-pack-abilities.py` 가 다시 만든다.
    /// </remarks>
    [Script("타겟어택", "5.99표")]
    public class SkillD0C0AC9FC5B4D0DD : SkillScript
    {
        public SkillD0C0AC9FC5B4D0DD(Skill skill) : base(skill)
        {
        }
        ...
        public override void OnUse(Sprite sprite)
        {
            ...
            {
                p.Call("message", (V)3L, (V)"사용하기에 마력량이적습니다. [필요마나 : 120]");
                return;
            }
            p.Call("manal_del", (V)120L);
            v_damage = ((V)(p.Call("get_att_damage", v_myid)) * (V)((V)7L));
            ...
            if (V.T(V.B(!V.T(v_target))))
            {
                if (V.T(V.B(V.T(((V)(p.Call("istype", p.Call("get_front_char", v_myid))) == (V)((V)3L))) && V.T(p.Call("get_map_pk")))))
                {
                    ...
                    p.Call("effect", v_target, (V)0L, (V)249L, (V)100L);
