...
{
    /// <summary>
    /// 가렌3 — 5.99 `Npc_Skill.txt` 의 NPC 스크립트를 그대로 옮긴 것.
    /// </summary>
    /// <remarks>
    /// 손으로 고치지 말 것. `scripts/build-pack-npcs.py` 가 다시 만든다.
    /// </remarks>
    [Script("NPC_가렌3", "5.99표")]
    public class NpcAC00B80C0033 : PackNpc
    {
        public NpcAC00B80C0033(GameServer server, Mundane mundane) : base(server, mundane)
        {
        }
        ...
        protected override IEnumerable<Prompt> Talk(Pack599 p, Reply reply)
        {
            ...
            {
                yield return Mes((V)1L, (V)"피닉스모드는 60초간 공격력을 상승시켜주는 버프기술 입니다.");
                ...
                if (V.T(p.Call("skill_exist", (V)"피닉스모드")))
                {
                    yield return Mes((V)1L, (V)"이미 이 스킬을 습득 하셧습니다.");
                    goto L_re;
                }
                p.Call("skill_add", (V)"피닉스모드");
                yield return Mes((V)1L, (V)"피닉스모드을 익히셧습니다. 앞으로 전사로서 사명감을 가져주시길 바랍니다.");
                goto L_re;
            }
            if (V.T(((V)(v_select) == (V)((V)2L))))
            {
                yield return Mes((V)1L, (V)"휘두르기는 앞의 몬스터들을 공격하는 기술입니다.");
                ...
                if (V.T(p.Call("skill_exist", (V)"휘두르기")))
                {
                    yield return Mes((V)1L, (V)"이미 이 스킬을 습득 하셧습니다.");
                    goto L_re;
                }
