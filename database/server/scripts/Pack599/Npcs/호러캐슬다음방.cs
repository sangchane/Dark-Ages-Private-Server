...
{
    /// <summary>
    /// 호러캐슬다음방 — 5.99 `Npc_Warp.txt` 의 NPC 스크립트를 그대로 옮긴 것.
    /// </summary>
    /// <remarks>
    /// 손으로 고치지 말 것. `scripts/build-pack-npcs.py` 가 다시 만든다.
    /// </remarks>
    [Script("NPC_호러캐슬다음방", "5.99표")]
    public class NpcD638B7ECCE90C2ACB2E4C74CBC29 : PackNpc
    {
        public NpcD638B7ECCE90C2ACB2E4C74CBC29(GameServer server, Mundane mundane) : base(server, mundane)
        {
        }
        ...
        protected override IEnumerable<Prompt> Talk(Pack599 p, Reply reply)
        {
            ...
            if (V.T(((V)(p.Call("map_user_exist", p.Call("get_mapname", v_myid))) == (V)((V)1L))))
            {
                p.Call("message", (V)3L, (V)"그룹원이 같은맵에없습니다.");
                p.Call("warp", (V)"호러캐슬대기실", (V)27L, (V)14L);
                yield break;
            }
            ...
            p.Call("item_clear", p.Call("get_mapname"));
            p.Call("mob_clear", p.Call("get_mapname"));
            p.Call("group_warp", p.Call("get_mapname"), (V)9L, (V)2L);
            if (V.T(V.B(V.T(((V)(p.Call("group_bighp", v_myid)) < (V)((V)50000L))) && V.T(((V)(p.Call("group_bigmp", v_myid)) < (V)((V)30000L))))))
            {
                p.Call("sleep", (V)300L);
