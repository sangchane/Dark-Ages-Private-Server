        ...
        private void ManageSpawns()
        {
            ...
            foreach (var map in ServerContext.GlobalMapCache.Values)
            {
                ...
                foreach (var template in temps)
                {
                    ...
                    // (2026-09-24, 사용자 결정). 경험치가 0 인 정의는 사냥감이 아니라 꾸밈이다(노비스마을의
                    // 노비스주민1·2 — 5.99 Npc_Spawn 에 한 명씩 적힌 걸어 다니는 주민). 넓이로 늘리면 죽지
                    // 않는 "괴물" 이 마을에 여섯 선다. SpawnMax 가 1 인 정의(포테의숲 사슴 같은 희귀 단독개체)도
